...
def _knn_idx(ref, query, k):
    """Blackwell-safe replacement for knn_cuda.KNN (removed for sm_120 / cu128).
    ref: (B, N, C), query: (B, G, C). Returns idx (B, G, k): for each query point,
    the indices of its k nearest neighbours in ref. Matches the (dist, idx) ->
    idx (B, G, k) layout that KNN(k, transpose_mode=True)(ref, query) produced.
    """
    dist = torch.cdist(query, ref)  # (B, G, N)
    return dist.topk(k, dim=-1, largest=False).indices  # (B, G, k)
...
class Group(nn.Module):
    ...
    def forward(self, xyz):
        ...
        # knn to get the neighborhood (cdist-based; see _knn_idx)
        idx = _knn_idx(xyz, center, self.group_size) # B G M
