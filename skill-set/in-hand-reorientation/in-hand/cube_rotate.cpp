...
// 2025.02.06.
// manual z-axis rotate

#define H5FILE_NAME "./data/test_int_long.hdf5"
#define DATASET_PATH "/data/demo_1/sim_joint_real"
#define SCALE_FACTOR 4096
#define SCALE_FACTOR2 2.1
// #define SCALE_FACTOR2 1.8
#define SCALE_FACTOR3 1.4

#define PI 3.14159265358979323846
#define UDP_PORT 12345
#define SERVER_IP "192.168.1.250"
#define FRAME_INTERVAL_MS 20  // 1/100초
...
int main() {
    try {
        ...
        for (size_t i = 0; i < dims[0]; ++i) {   // 새끼 구부리기
            ...
            // raw_data[index] *= 0.95;  // 해당 값만 변경
            raw_data[index] *= 1;  // 해당 값만 변경
        }
        for (size_t i = 0; i < dims[0]; ++i) {  
        ...
        for (size_t i = 0; i < raw_data.size(); ++i) {
            processed_data[i] = static_cast<int16_t>((raw_data[i] * SCALE_FACTOR * SCALE_FACTOR2) / PI *1.12);
        }
        ...
        oss << folder << "/" << "Cube_"
            << now->tm_hour << "_" << now->tm_min << "_" << now->tm_sec << ".hdf5";
        ...
        if (row_size < 111) {
            std::cerr << "Error: Received data row size is " << row_size << ", which is less than the expected 104." << std::endl;
            return 1;
        }
