...
동작 순서:
  ...
  3) 실제 작업(블로킹, 완료까지 대기):
       pose_commander.py 로 목표 pose 이동 → 2초 → hand_manual_squeeze ('+' 2회 오무리기)
...
# 이동 완료 + 2초 후 실행할 hand manual squeeze (오무리기 2회 후 종료)
...
SQUEEZE_PRESSES = 2          # '+' 입력 횟수 (오무리기 2회)
...
def do_inhand_work(run_command, hand_side):
    """Start~End 사이의 실제 in-hand 작업 (블로킹, 완료까지 대기)."""
    print('in-hand manipulation start', flush=True)
    if not run_command:
        return
    # 1) pose_commander 실행 → 로봇 모션 완료까지 대기 (프로세스 종료 = 이동 완료)
    _run_and_wait(POSE_COMMANDER_CMD, 'pose_commander.py', stdin_text=POSE_COMMANDER_STDIN)
    # 2) 이동 완료 후 2초 대기
    print(f'[inhand] 이동 완료 → {POST_MOVE_DELAY:.0f}초 후 hand squeeze', flush=True)
    time.sleep(POST_MOVE_DELAY)
    # 3) hand_manual_squeeze 실행 → '-' 2회로 오무린 뒤 종료까지 대기
    run_hand_squeeze(hand_side)
...
def main():
    ...
    try:
        ...
        with client:  # Start(S): request_control → RUNNING + 하트비트
            do_inhand_work(run_command=not args.print_only, hand_side=args.hand_side)
