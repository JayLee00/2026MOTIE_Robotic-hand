...
def move_hand_to_squeeze(
    ...
    # 함수 시작 시점의 현재 손 위치 — thumb 외 손가락은 스퀴즈 구간 내내 이 위치를 유지.
    msg = shm.read()
    start_pos = [int(msg.j_pos[0][j]) for j in range(Hand_DOF)]

    # 스퀴즈 target: thumb(SQUEEZE_FORCE_FINGERS) 관절만 target_position(HAND_SAVE_POINT)으로
    # 닫고, 나머지 손가락은 start_pos 그대로 둔다(target==start → 보간해도 안 움직임).
    squeeze_target = list(start_pos)
    ...
    try:
        # 1) 스퀴즈: thumb 만 닫고 나머지 손가락은 start_pos 유지, threshold 도달 position 을 받아둔다.
    ...
    restore = list(squeezed)                       # 현재(스퀴즈) 위치에서 시작
    for f in SQUEEZE_FORCE_FINGERS:                 # 스퀴즈로 움직인 finger(=thumb) 만
        for j in range(f * JOINTS_PER_FINGER, (f + 1) * JOINTS_PER_FINGER):
            restore[j] = return_position[j]         # 그 finger 관절만 파지 위치로 되돌림
    move_hand_to(shm, restore, return_duration)
