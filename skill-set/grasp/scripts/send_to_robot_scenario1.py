#!/usr/bin/env python3
"""
시나리오1용 — 원본 send_to_robot.py 의 복사본.
robot_executor.py 대신 robot_executor_scenario1.py(물체 쥔 채 유지)를 컨테이너에서 실행한다.
원본 send_to_robot.py 는 건드리지 않는다.

Usage:
    python scripts/send_to_robot_scenario1.py --summary_json data/outputs/xxx_topdown_summary.json
"""
...
def main():
    ...
    # ← 원본과 다른 부분: robot_executor_scenario1.py 실행
    executor_ctr  = to_container_path(str(SCRIPTS / "robot_executor_scenario1.py"))
    summary_ctr   = to_container_path(summary_host)
    kistar_ws_ctr = to_container_path(args.kistar_ws)

    print(f"[send_to_robot_scenario1] Docker exec → {args.container}  mode={mode} (물체 쥔 채 유지)")
    ...
    if rc == 0:
        print(f"\n[send_to_robot_scenario1] {mode} 완료 (물체 유지).")
    else:
        print(f"\n[send_to_robot_scenario1] 종료 코드: {rc}")
    sys.exit(rc)
